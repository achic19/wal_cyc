...
def create_wkt_and_index(engine):
    ...
    for table_name in inspector.get_table_names(schema=PRODUCTION_SCHEME):
        table = gpd.GeoDataFrame.from_postgis(f'select * from {PRODUCTION_SCHEME}.{table_name}', engine, geom_col='geometry')
        ...
        if not is_table_has_spatial_index(table_name, inspector):
            with engine.connect() as connection:
                result = connection.execute(text(f"CREATE INDEX ON {PRODUCTION_SCHEME}.{table_name} using gist (geometry)"))
